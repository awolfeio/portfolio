...
// Disabled: import { initZoomLogic, setupMagnifyingGlass } from "./cursor-zoom";
import { initMoreInfoLogic, setupMoreInfo } from "./cursor-more-info";

gsap.registerPlugin(CustomEase);

// PERFORMANCE: Create custom ease ONCE at module load, not per-mousemove
export const cursorEase = CustomEase.create("cursorEase", ".14,.8,.28,.99");
...
export let currentMousePos = { x: -100, y: -100 };
...
export function cursorElement() {
  document.addEventListener("DOMContentLoaded", function () {
    ...
    // Initialize specific cursor features
    initMoreInfoLogic();
    // initZoomLogic(); // Zoom feature disabled as per request
...
// Disabled zoom logic has been moved to cursor-zoom.js
// Export the setup function so it can be called elsewhere if needed
export { setupMoreInfo };
